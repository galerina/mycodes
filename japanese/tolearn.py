...
jlpt_filename = "jlpt.json"
with open(jlpt_filename) as jlpt_file:
    jlpt_kanji = json.load(jlpt_file)
for kanji in jlpt_kanji:
    if not kanji in anki_kanji:
        new_kanji.append(kanji)

noruwei_filename = "noruwei.json"
with open(noruwei_filename) as noruwei_file:
    noruwei_kanji = json.load(noruwei_file)
for kanji in noruwei_kanji[:100]:
    if not kanji in anki_kanji:
        new_kanji.append(kanji)

new_kanji.sort(key=lambda x: heisig_index_map[x])
print "%d new kanji:"%len(new_kanji)
